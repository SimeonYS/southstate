...
class SouthstatePipeline:

    # Database setup
    conn = sqlite3.connect('southstate.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `southstate`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM southstate WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO southstate (date, title, link, content)"
